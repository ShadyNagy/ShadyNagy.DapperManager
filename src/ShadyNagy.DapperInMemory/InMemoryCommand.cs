...
namespace ShadyNagy.DapperInMemory
{
  internal class InMemoryCommand : IDbCommand
  {
    ...
    public int ExecuteNonQuery()
    {
      ...
      {
        if (GetSqlCommandType() == SqlSyntaxType.Nothing)
        {
          return 0;
        }
    ...
    private SqlSyntaxType GetSqlCommandType()
    {
      if (string.IsNullOrEmpty(CommandText))
      {
        return SqlSyntaxType.Nothing;
      }
      ...
      {
        return SqlSyntaxType.Select;
      }
      else if (parts[0].ToLower() == "insert")
      {
        return SqlSyntaxType.Insert;
      }
      else if (parts[0].ToLower() == "update")
      {
        return SqlSyntaxType.Update;
      }
      else if (parts[0].ToLower() == "create" && parts[1].ToLower() == "table")
      {
        return SqlSyntaxType.CreateTable;
      }
