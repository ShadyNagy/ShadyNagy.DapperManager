  ...
  public class MicrosoftDapperService : IDapperService
  {
    ...
    public List<T> GetByIdsFrom<T>(string name, Dictionary<string, object> fields)
    {
      try
      {
        ...
        return (connection.Query<T>(_microsoftSyntaxBuilder.SelectByFrom(name, fields).Build(), commandType: CommandType.Text)).ToList();
      }
      catch (Exception exception)
      {
        _logger.LogError(exception.Message);
    ...
    public async Task<List<T>> GetByIdsFromAsync<T>(string name, Dictionary<string, object> fields)
    {
      try
      {
        ...
        return (await connection.QueryAsync<T>(_microsoftSyntaxBuilder.SelectByFrom(name, fields).Build(), commandType: CommandType.Text)).ToList();
      }
      catch (Exception exception)
      {
        _logger.LogError(exception.Message);
    ...
    public async Task<List<T>> GetByIdsFromSafeAsync<T>(string name, Dictionary<string, string> fields, object idsObject)
    {
      try
      {
        ...
        return (await connection.QueryAsync<T>(_microsoftSyntaxBuilder.SelectByFromSafe(name, fields).Build(), idsObject, commandType: CommandType.Text)).ToList();
      }
      catch (Exception exception)
      {
        _logger.LogError(exception.Message);
